...
def racunanjeOdnosa(slika,detector,predictor):
...
def odrediStarost(slika1):
    ...
    jedanUlaz=racunanjeOdnosa(slika1,detector,predictor)
    ...
    with open('mrezaStarostDveGrupeV2','rb') as f:
        novi = pickle.load(f)
        t = novi.predict(sviUlazi1, verbose=1)
        ind = np.argmax(t)
        if ind==0:
            return "DETE"
        else:
...
'''for ii in xrange(1,121):
    if ii!=11:
        putanja1 = "Dataset_Starost/Mladi/"+str(ii)+".png"
        lice=cv2.imread(putanja1)
        jedanUlaz=racunanjeOdnosa(lice,detector,predictor)
        #print ii
        sviUlazi.append(jedanUlaz)
for ii in xrange(1,121):
    if ii!=11:
        putanja1 = "Dataset_Starost/Stari/"+str(ii)+".png"
        lice=cv2.imread(putanja1)
        jedanUlaz=racunanjeOdnosa(lice,detector,predictor)
        #print ii
        sviUlazi.append(jedanUlaz)
